...
switch($op){
    ...
    case 'delete':
        $aid = I($_POST['aid']);
        $aid = $aid?$aid:$_GET['aid'];
        echo $sql = "update cate set isdel=1 where aid='{$aid}'";
        $re = update($sql);
        if($re){
            go(U());
        }else{
    ...
    case 'edit';
        $_VAL = I($_POST);
        if($type == 'do'){
            $sql = "update cate set cname='{$_VAL['name']}',sort='{$_VAL['sort']}',cdesc='{$_VAL['cdesc']}' where aid='{$_VAL['aid']}'";
            $re = update($sql);
            if($re !== false){
                go(U());
            }else{
