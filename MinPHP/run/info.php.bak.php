...
<?php
   ...
   //修改接口
       ...
       if($type == 'do'){
           ...
           parameter='{$parameter}',re='{$re}',lasttime='{$lasttime}',lastuid='{$lastuid}',memo='{$memo}',gid='{$gid}'
           where id = '{$id}'";
           $re = update($sql);
           if($re){
   ...
   //此分类下的接口列表
        ...
        $sql = "select api.id,aid,num,url,name,des,parameter,memo,re,lasttime,lastuid,type,login_name,nice_name
        from api
        left join user
                ...
                <form action="?act=api&tag=<?php echo $_GET['tag']?>&type=do&op=edit" method="post">
                    ...
                    <div class="form-group" required="required">
                        <select class="form-control" name="gid">
                            ...
                            <?php
                                foreach ($group_list as $key => $value) {
                                    echo '<option value="'.$value['id'].'" '.($info['gid']==$value['id']?'selected=""':"").'>'.$value['name'].'</option>';
                                }
                            ?>
                        </select>
                    </div>
